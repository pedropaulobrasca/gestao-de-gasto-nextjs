...
export default function EditExpense({ userId, open, data }: Props) {
  ...
  useEffect(() => {
    data.installments > 0 && setIsInstallments(!isInstallments);
  }, [data]);
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      repeatExpense: Boolean(data.repeatExpense),
    },
  });
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogTrigger onClick={() => setIsDialogOpen(true)}>Edit</DialogTrigger>
      <DialogContent>
        <DialogHeader>
